...
class Parser(object):
    ...
    def factor(self):
        """factor : INTEGER | OPEN_BRACKET expr CLOSE_BRACKET
        returns a single factor from which we will calculate. it can either be a number or an
        expression in parentheses, which sooner or later results in a number and that number is used to
        further calculate the expression this function was called from"""
        ...
        if token.type == INTEGER:
            ...
            node = self.expr()
            self.eat(CLOSING_BRACKET)
            return node

    def term(self):
        """term : factor ((MULTIPLICATION | DIVISION) factor)*
        meaning it checks if our next term is a number or an operation in of itself and returns what's appropriate
        """
        node = self.factor()
